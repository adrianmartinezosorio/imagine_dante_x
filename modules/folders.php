...
function folder_full_list_in_array_cachewrite_jpgs($ruta,$contenido,$metodo = 'w'){
...
function folder_full_list_process_jpgs($ruta,$uniqidname)
     {
     // abrir un directorio y listarlo recursivo
     if (is_dir($ruta))
     {
     ...
     if(is_file($ruta.$file) && file_exists($ruta.$file) && !empty($ruta.$file) && exif_imagetype($ruta.$file) == IMAGETYPE_JPEG){


        logg('ADD_SPIDER_'.$file,'../');
        folder_full_list_in_array_cachewrite_jpgs($uniqidname.'.txt',$ruta.$file.'+X####--Y1Y--####X+','a');
        //echo $ruta.$file.'+X####--Y1Y--####X+<br>';


     }

     folder_full_list_process_jpgs($ruta . $file . '/',$uniqidname); // Ahora volvemos a llamar la función
     }
     }
     closedir($dh);
     }
...
function folder_full_list_jpgs($path){

            logg('INIT_SPIDER_'.$path,'../');

             $uniqidname = uniqid().uniqid();

             logg('UNIQIDNAME_'.$uniqidname,'../');

             folder_full_list_process_jpgs($path,$uniqidname);
